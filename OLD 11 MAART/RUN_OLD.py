...
async def my_ask():
    ...
    while TICKER_PERP == {}:
        await asyncio.sleep(0.00001)
    while True:
        perp_bid = TICKER_PERP["bid"]
        perp_ask = TICKER_PERP["ask"]
...
async def my_spot_exit_ask():
    ...
    while True:
        perp_bid = TICKER_PERP["bid"]
        perp_ask = TICKER_PERP["ask"]  # WHEN PERP MARKET EXIT (SPOT BASED ON PERP_ASK INSTEAD OF PERP_BID) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        spot_ask = TICKER_SPOT["ask"]

        perp_bid_plus_bps = perp_bid * 1.0003   # 3 bps (0.03%) above best bid we sell spot (to have some room for perp volatility inbetween spot close and perp close)
        if perp_bid_plus_bps > spot_ask:
...
async def my_perp_exit_bid():
    ...
    while True:
        ...
        if perp_ask > (perp_bid + price_increment_perp):
            my_perp_bid = perp_bid + price_increment_perp
        else:
            my_perp_bid = perp_bid
...
async def order_execution():
    ...
    while float(ORDERSTATUS["filledSize"] == 0):
        ...
        if (ORDERSTATUS["status"] == "closed") and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
            ...
            while ORDERSTATUS["status"] != "new" and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):  # BELANGRIJK! W8 OP ORDER BEVESTIGING
                await asyncio.sleep(0.00001)
    ...
    while (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
        await asyncio.sleep(0.00001)
        if (ORDERSTATUS["price"] != TICKER_PERP["ask"]) and (ORDERSTATUS["status"] != "closed") \
        ...
        if (ORDERSTATUS["status"] == "closed") and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
            client.place_order(market=f"{future_market}", side="sell", price=MY_ASK, type="limit",
                                       size=(ORDERSTATUS["size"] - ORDERSTATUS["filledSize"]), post_only=True,reduce_only=False)
            while ORDERSTATUS["status"] != "new" and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):  # BELANGRIJK!!!!!!! EVEN WACHTEN TOT WEBSOCKET NEW ORDER BEVESTIGT
                await asyncio.sleep(0.00001)
    ...
    while (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
        ...
        if (ORDERSTATUS["status"] == "closed") and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
            client.place_order(market=f"{spot_market}", side="sell", price=MY_EXIT_ASK, type="limit", size=(ORDERSTATUS["size"] - ORDERSTATUS["filledSize"]), post_only=True, reduce_only=True)
    ...
    while (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
        ...
        if (ORDERSTATUS["status"] == "closed") and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
            client.place_order(market=f"{future_market}", side="buy",price=MY_EXIT_BID,type="limit", size=(ORDERSTATUS["size"] - ORDERSTATUS["filledSize"]),post_only=True,reduce_only=True)
            while ORDERSTATUS["status"] != "new" and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):  # BELANGRIJK!!!!!!! EVEN WACHTEN TOT WEBSOCKET NEW ORDER BEVESTIGT
                await asyncio.sleep(0.00001)
...
async def main():
    ...
    await asyncio.gather(websocket(json.dumps(msg)), my_ask(), my_bid(), order_execution(), my_spot_exit_ask(), my_perp_exit_bid())
