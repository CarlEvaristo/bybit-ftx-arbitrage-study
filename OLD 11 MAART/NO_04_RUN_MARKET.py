...
from NO_01_FIND_COINS_HIGH_FUNDING import highest_predicted_funding_coin
import logging
...
coin = highest_predicted_funding_coin()
...
async def my_bid():
    ...
    while TICKER_PERP == {} or TICKER_SPOT == {}:
        await asyncio.sleep(0.00001)
    while True:
        spread = TICKER_PERP["bid"] - TICKER_SPOT["bid"]
        ...
        if spread_array.size > minimum_items:
            ...
            my_bid_price = TICKER_PERP["bid"] - mean_plus_st_dev
            if my_bid_price > (TICKER_PERP["bid"] * 0.9993) :  # 7 BASISPOINTS  0.07% (0.07% / 100 = 0.0007 --> 1 - 0.0007 = 0.9993)
                my_bid_price = (TICKER_PERP["bid"] * 0.9993)
...
async def my_spot_exit_ask():
    ...
    while TICKER_PERP == {} or TICKER_SPOT == {}:
        await asyncio.sleep(0.00001)
    while True:
        # TRYING TO SELL SPOT ABOVE PERP BEST ASK (MARKET CLOSE BUY PERP -> CROSS SPREAD -> PERP BEST ASK)
        # 3 bps (0.03%) try to sell spot above perp best ask (market close buy) (to have some room for perp volatility inbetween spot close and perp close)
        perp_ask_plus_bps = TICKER_PERP["ask"] * 1.0014   # 14 BASISPOINTS 0.14% / 100 = 0.0014
        if perp_ask_plus_bps > TICKER_SPOT["ask"]:
            my_spot_ask = perp_ask_plus_bps
        else:
...
async def order_execution():
    ...
    while MY_BID == 0:
        print("Waiting for sufficient data to calculate my bid price")
        await asyncio.sleep(0.00001)

    # determine position size
    balance = client.get_balances()
    available_USD = [item["free"] for item in balance if item["coin"] == "USD"][0]
    available_USD = (available_USD * 0.90) / 2  # takes 95% of subaccount and divides it 50/50 over spot/perp
    ...
    while float(ORDERSTATUS["filledSize"] == 0):
        await asyncio.sleep(0.00001)
        if ((MY_BID > (ORDERSTATUS["price"] + price_increment_spot)) or (
                MY_BID < (ORDERSTATUS["price"] - price_increment_spot))) \
    ...
    # PERP MARKET ORDER
    perp_order = client.place_order(market=f"{future_market}", side="sell", price=0, type="market",
                                    size=spot_size, reduce_only=False)
    print("SPOT ORDER FILLED. SENDING PERP MARKET SELL ORDER.")
    time.sleep(2)
    print(f"PERP MARKET ORDER: {perp_order}")
    ...
    while (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
        await asyncio.sleep(0.00001)
        if ((MY_EXIT_ASK > (ORDERSTATUS["price"] + price_increment_spot)) or (
                MY_EXIT_ASK < (ORDERSTATUS["price"] - price_increment_spot))) \
                and (ORDERSTATUS["status"] != "closed") and (ORDERSTATUS["filledSize"] != ORDERSTATUS["size"]):
    ...
    # PERP LIMIT EXIT -------------------------------------------------------------------------------------------
    # PERP MARKET ORDER
    perp_order = client.place_order(market=f"{future_market}", side="buy", price=0, type="market", size=spot_size, reduce_only=True)
    print("SPOT EXIT FILLED. SENDING PERP MARKET EXIT ORDER.")
    time.sleep(2)
    print(f"PERP MARKET EXIT ORDER: {perp_order}")
...
async def main():
    ...
    await asyncio.gather(websocket(json.dumps(msg)), my_bid(), order_execution(), my_spot_exit_ask())
