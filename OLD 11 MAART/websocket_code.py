...
class FtxWebsocketClient(WebsocketManager):
    ...
    def __init__(self, api = None, secret = None, subaccount = None) -> None:
        super().__init__()
    ...
    def _login(self) -> None:
        ...
        signature = hmac.new(self._api_secret.encode(), f'{ts}websocket_login'.encode(), 'sha256').hexdigest()
        self.send_json({'op': 'login', 'args': {'key': self._api_key, 'sign': signature, 'time': ts, 'subaccount': self._subaccount,  }})
